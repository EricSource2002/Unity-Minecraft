...
{
    public bool reloadTerrain = false;
    private bool isReadyForLoad = true;
    public Transform player;
    public GameObject terrainChunkPrefab;

    private static Dictionary<ChunkPos, TerrainChunkObject> chunks = new Dictionary<ChunkPos, TerrainChunkObject>();
    public static Dictionary<ChunkPos, Dictionary<Vector3, BlockType>> holdsToGenerate = new Dictionary<ChunkPos, Dictionary<Vector3, BlockType>>();

    // Start is called before the first frame update
    void Start()
    {
        ChunkGenerator.ChunkGenerator.Start();
        LoadChunks(1, true);
    }
    private void Update()
    {
    ...
    void RefreshChunks()
    {
        List<ChunkPos> removeCp = new List<ChunkPos>();
        foreach (KeyValuePair<ChunkPos, Dictionary<Vector3, BlockType>> chunk in holdsToGenerate)
        {
            if (!chunks.ContainsKey(new ChunkPos(chunk.Key.x, chunk.Key.z))) return;
            TerrainGenerator.updateChunk(new ChunkPos(chunk.Key.x, chunk.Key.z), (blocks, updateBlock) =>
            {
                foreach (var block in holdsToGenerate[chunk.Key])
                {
                    updateBlock((int)block.Key.x, (int)block.Key.y, (int)block.Key.z, block.Value);
                }
                removeCp.Add(new ChunkPos(chunk.Key.x, chunk.Key.z));
            });
        }
        foreach (ChunkPos cp in removeCp)
        {
            holdsToGenerate.Remove(cp);
        }
    ...
    void ReRender()
    {
        ...
        ChunkGenerator.ChunkGenerator.chunkBlocks.Clear();
        ChunkGenerator.ChunkGenerator.requestedChunk.Clear();
        chunks.Clear();
        ChunkGenerator.ChunkGenerator.Start();
        isReadyForLoad = true;
        print("TERRAIN RELOADED");
    }
    ...
    TerrainChunkObject LoadChunk(ChunkPos cp, bool instant)
    {
        ...
        Chunk chunk = ChunkGenerator.ChunkGenerator.request(cp, instant);
        if (chunk == null)
        {
            return null;
        }
        ...
        chunkObject.name = "Chunk_" + cp;

        chunkObject.BuildMesh(chunk);

        WaterChunkObject water = chunkObject.transform.GetComponentInChildren<WaterChunkObject>();
        water.SetLocs(chunk.blocks, chunkObject.Chunk);

        water.BuildMesh();

        chunks.Add(cp, chunkObject);
       // Generation.GenerationManager.AddToStructureQue(cp);
        return chunkObject;
    }
    public static BlockType getBlock(ChunkPos pos, int x, int y, int z)
    {
        ...
        {
            x += Chunk.chunkWidth;
            pos = new ChunkPos(pos.x - Chunk.chunkWidth, pos.z);
        }
        ...
        {
            z += Chunk.chunkWidth;
            pos = new ChunkPos(pos.x, pos.z - Chunk.chunkWidth);
        }
    ...
    public static void updateChunk(ChunkPos cp, Action<BlockType[,,], Action<int, int, int, BlockType>> cb)
    {
        ...
        cb(blocks, (x, y, z, block) =>
        {
            ...
            if (x == 1)
            {
                updateChunkNearbyHelper(new ChunkPos(cp.x - Chunk.chunkWidth, cp.z), changed, Chunk.chunkWidth + 1, y, z, block);
            }
            if (z == 1)
            {
                updateChunkNearbyHelper(new ChunkPos(cp.x, cp.z - Chunk.chunkWidth), changed, x, y, Chunk.chunkWidth + 1, block);
            }
            if (x == 1 + Chunk.chunkWidth - 1)
            {
                updateChunkNearbyHelper(new ChunkPos(cp.x + Chunk.chunkWidth, cp.z), changed, 0, y, z, block);
            }
            if (z == 1 + Chunk.chunkWidth - 1)
            {
                updateChunkNearbyHelper(new ChunkPos(cp.x, cp.z + Chunk.chunkWidth), changed, x, y, 0, block);
            }
        });
    ...
    {
        if (!chunks.ContainsKey(cp)) return;
    ...
    void LoadChunks(int chunkDist, bool instant = false)
    {
        ...
        {
            if (ChunkGenerator.ChunkGenerator.getRequestSize() < 5 || instant)
            {
